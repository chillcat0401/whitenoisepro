...
2026-06-11 规则对账(华为侧五条上架要求):
- **ICP 备案:单机应用豁免**——本应用纯本地零联网,首发按单机申报免备案;
  未来任何联网能力(广告/订阅/内容下载)上线前必须先完成备案(含鸿蒙平台),
  变现时间表需前置 1~4 周备案期;
- 应用名称须与(将来的)备案名称一致 → 现在锁定「白噪屿」(2026-06-11 用户确认;包名 com.whitenoisepro.hmos 确认);
- 鸿蒙当前不强制软著(建议并行申请:IP 保护 + 后续安卓国内商店需要);
- **鸿蒙包名须与安卓不同**:安卓 com.whitenoisepro → 鸿蒙 com.whitenoisepro.hmos
  (壳定制 0003 补丁);
- 个人开发者可上架免费应用;IAP/广告需企业资质(既有结论)。
